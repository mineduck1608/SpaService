...
export default function UpdateAccountModal({ isOpen, onClose, account }: UpdateAccountModalProps) {
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (selectedRole) data.role = selectedRole.roleId
    data.status = data.status === 'Active'
    handleUpdateSubmit(account, data)
  }
      ...
      if (account) {
        Object.keys(account).forEach((key: string) => {
          if (form.getValues(key) !== undefined) {
            ...
            } else if (key === 'status') form.setValue('status', account.status ? 'Active' : 'Locked')
            else form.setValue(key, account[key])
          }
        })
      }
    }
