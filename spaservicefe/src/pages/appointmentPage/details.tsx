  ...
  useEffect(() => {
    const checkEligibility = async () => {
      ...
      if (customerId && request.appointmentId) {
        const feedbackExists = await hasSendFeedback(request.request?.service?.serviceId, customerId)
        setHasFeedback(feedbackExists)
      }
    }
    checkEligibility()
  }, [request.request?.service?.serviceId])
        ...
        <>
          {!hasFeedback ? (
            <>
              ...
              <FeedbackModal isOpen={isFeedbackModalOpen} onClose={closeFeedbackModal} service={request.request?.service}/>
            </>
          ) : (
            <>
              ...
              <UpdateFeedbackModal isOpen={isUpdateFeedbackModalOpen} onClose={closeUpdateFeedbackModal} service={request.request?.service} />
            </>
          )}
        </>
      )}
